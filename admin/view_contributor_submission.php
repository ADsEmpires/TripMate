...
<div class="main-content">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
        <div>
            ...
            <h1 style="font-size: 2.2rem; color: var(--text-primary); margin-top: 10px;">Package Review: <?= htmlspecialchars($destination['name']) ?></h1>
        </div>
        <div>
            <?php 
            $status = $destination['submission_status'];
        ...
        <div style="display: flex; gap: 25px; flex-wrap: wrap;">
            ...
            <div style="flex: 2; min-width: 300px;">
                <div class="info-grid">
                    ...
                    <div class="info-item"><label>Type</label><div><?= ucfirst($destination['type']) ?></div></div>
                    <div class="info-item"><label>Location</label><div><?= htmlspecialchars($destination['location']) ?></div></div>
                    <div class="info-item"><label>Budget (Per Day)</label><div>₹<?= number_format($destination['budget']) ?></div></div>
                    <div class="info-item"><label>Best Season</label><div><?= ucwords($destination['season']) ?></div></div>
                    <div class="info-item"><label>Recommended For</label><div><?= $destination['people'] ?></div></div>
                </div>
                ...
                <?php foreach ($flights as $f): ?>
                    <div class="sub-card">
                        <div style="display: flex; justify-content: space-between; border-bottom: 1px solid var(--border); padding-bottom: 10px; margin-bottom: 10px;">
                            <strong style="color: var(--text-primary); font-size: 1.1rem;"><?= htmlspecialchars($f['airline']) ?></strong>
                            <span style="color: #27ae60; font-weight: bold;">₹<?= number_format($f['price_per_person']) ?></span>
                        </div>
                        <div style="color: var(--text-muted); font-size: 0.9rem; line-height: 1.5;">
                            <strong>From:</strong> <?= htmlspecialchars($f['departure_city']) ?><br>
                            <strong>Type:</strong> <?= ucfirst($f['flight_type']) ?> | <strong>Class:</strong> <?= htmlspecialchars($f['flight_class']) ?><br>
                            <strong>Time:</strong> <?= htmlspecialchars($f['departure_time']) ?> to <?= htmlspecialchars($f['arrival_time']) ?><br>
                            <strong>Duration:</strong> <?= $f['duration_hours'] ?> hrs (<?= $f['stops'] ?> stops)<br>
                            <div style="margin-top: 5px;">
                                <?= $f['refundable'] ? '<span style="background: #d4efdf; color: #00b894; padding: 2px 6px; border-radius: 4px; font-size: 0.8rem;">Refundable</span>' : '' ?>
                                <?= $f['meal_included'] ? '<span style="background: #ffeaa7; color: #d35400; padding: 2px 6px; border-radius: 4px; font-size: 0.8rem;">Meal</span>' : '' ?>
                            </div>
                        </div>
                    </div>
                <?php endforeach; ?>
        ...
        <?php else: ?>
            <div class="card-grid">
                <?php foreach ($hotels as $h): ?>
                    <div class="sub-card">
                        <div style="display: flex; justify-content: space-between; border-bottom: 1px solid var(--border); padding-bottom: 10px; margin-bottom: 10px;">
                            <strong style="color: var(--text-primary); font-size: 1.1rem;"><?= htmlspecialchars($h['hotel_name']) ?> <span style="color: #f1c40f; font-size: 0.9rem;"><i class="fas fa-star"></i> <?= $h['hotel_rating'] ?></span></strong>
                            <span style="color: #27ae60; font-weight: bold;">₹<?= number_format($h['price_per_night']) ?>/nt</span>
                        </div>
                        <div style="color: var(--text-muted); font-size: 0.9rem; line-height: 1.5;">
                            <strong>Type:</strong> <?= ucfirst($h['hotel_type']) ?><br>
                            <strong>Address:</strong> <?= htmlspecialchars($h['address']) ?><br>
                            <strong>Contact:</strong> <?= htmlspecialchars($h['contact_number']) ?><br>
                            <strong>Times:</strong> In: <?= substr($h['check_in_time'], 0, 5) ?> | Out: <?= substr($h['check_out_time'], 0, 5) ?><br>
                            <div style="margin-top: 5px;">
                                <?= $h['free_cancellation'] ? '<span style="background: #d4efdf; color: #00b894; padding: 2px 6px; border-radius: 4px; font-size: 0.8rem;">Free Cancel</span>' : '' ?>
                                <?= $h['breakfast_included'] ? '<span style="background: #ffeaa7; color: #d35400; padding: 2px 6px; border-radius: 4px; font-size: 0.8rem;">Breakfast</span>' : '' ?>
                            </div>
                        </div>
                    </div>
                <?php endforeach; ?>
    ...
    <div style="display: flex; justify-content: flex-end; gap: 15px; margin-top: 30px; border-top: 1px solid var(--border); padding-top: 20px;">
        ...
        <?php else: ?>
            <span style="color: var(--text-muted); font-style: italic;">This package has already been <?= $destination['submission_status'] ?>.</span>
        <?php endif; ?>
    </div>
