...
// Get all destinations for dropdown with processed images
$destinations = [];
$dest_stmt = $conn->prepare("SELECT id, name, location, type, budget, images, profile_pic FROM destinations ORDER BY name");
$dest_stmt->execute();
$dest_result = $dest_stmt->get_result();
while ($row = $dest_result->fetch_assoc()) {
    // ============================================
    // FIXED: Process image for each destination
    // ============================================
    ...
    if (!empty($row['images'])) {
        ...
        if (is_array($decoded_images) && !empty($decoded_images)) {
            $cover_image = $base_url . '/uploads/destinations/' . basename($decoded_images[0]);
        }
    }
...
<body>
        ...
        <div class="dashboard-grid">
            ...
            <div class="glass-card">
                <div class="flex justify-between items-center mb-4">
                    ...
                    <button class="view-all-btn" onclick="openReviewModal()">
    ...
    <script>
        ...
        function getDestinationImageUrl(filename, baseUrl = BASE_URL) {
            ...
            filename = filename.replace(/^\/+/, '').replace(/^(\.\.\/)+/, '').replace(/^uploads\//, '').replace(/^destinations\//, '');
            return baseUrl + '/uploads/destinations/' + filename;
        }

        function renderRecommendationCard(dest) {
            // FIXED: Properly handle image URL
            let imgUrl = BASE_URL + '/images/no-image.jpg';

            // Check if we have cover_image from PHP (already processed)
            if (dest.cover_image) {
                imgUrl = dest.cover_image;
            }
            // Fallback to images array
            else if (dest.images) {
                let rawImg = dest.images;
                if (typeof rawImg === 'string') {
                    try {
                        let parsed = JSON.parse(rawImg);
                        if (Array.isArray(parsed) && parsed.length) {
                            imgUrl = BASE_URL + '/uploads/destinations/' + basename(parsed[0]);
                        }
                    } catch (e) {
                        imgUrl = BASE_URL + '/uploads/destinations/' + basename(rawImg);
                    }
                } else if (Array.isArray(rawImg) && rawImg.length) {
                    imgUrl = BASE_URL + '/uploads/destinations/' + basename(rawImg[0]);
                }
            }
            // Fallback to profile_pic
            else if (dest.profile_pic) {
                imgUrl = BASE_URL + '/uploads/destinations/' + basename(dest.profile_pic);
            }
            ...
            card.innerHTML = `
                <div class="recommendation-image" style="background-image: url('${escapeHtml(imgUrl)}');"></div>
                <div class="recommendation-content">
                    <div class="recommendation-name">${escapeHtml(dest.name || 'Untitled')}</div>
                    <div class="recommendation-location">
                        <i class="fas fa-map-marker-alt"></i> ${escapeHtml(dest.location || '')}
                    </div>
                    <div class="recommendation-price">₹${Number(dest.budget || 0).toLocaleString()}/day</div>
                </div>
            `;
            return card;
        }

        // Helper function for basename (if not already defined)
                ...
                .then(data => {
                    if (data.status === 'success' && data.recommendations.length > 0) {
                        container.innerHTML = data.recommendations.map(dest => {
                            let imageUrl = BASE_URL + '/images/no-image.jpg';
                            if (dest.image_urls && dest.image_urls.length > 0) {
                                imageUrl = getDestinationImageUrl(dest.image_urls[0]);
                            }
                            return `
        ...
        function submitReview(event) {
            ...
            formData.append('rating', selectedRating ? selectedRating.value : 0);
        ...
        async function quickSearch(query) {
            ...
            // Check if the query matches any destination name in the database
            try {
                ...
                if (data.status === 'success' && data.destination_id) {
                    ...
                    // Show destination not found and redirect to search page with the query
                    showToast(`"${query}" not found. Showing search results.`, 'warning');
                    setTimeout(() => {
        ...
        function viewDestination(destinationId) {
            window.location.href = '../user/destination_details.php?id=' + destinationId;
        }
