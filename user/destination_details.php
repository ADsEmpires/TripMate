...
body{margin:0;font-family:Inter, 'Segoe UI', Roboto, Arial, sans-serif;background:linear-gradient(135deg,#f7f8fc 55%,#e9f0fb 100%);color:#222;line-height:1.45}
...
.container{
    max-width:1200px;
    margin:0 auto;
    padding: calc(64px + 24px) 16px 48px; /* top padding includes navbar height */
}

/* Navbar */
.navbar{position:fixed;left:0;right:0;top:0;height:64px;display:flex;align-items:center;justify-content:space-between;padding:0 16px;background:linear-gradient(90deg, rgba(22,3,79,0.96), rgba(16,2,82,0.96));color:#fff;z-index:1200}
.navbar .logo{display:flex;gap:10px;align-items:center;font-weight:700}
.navbar a.btn{background:var(--accent);padding:8px 12px;border-radius:8px;color:#fff;font-weight:600}

/* Hero / cover with Ken Burns (slow zoom) */
/* Use viewport-based heights but clamp to reasonable min/max to fit various screens */
.hero{position:relative;border-radius:12px;overflow:hidden;box-shadow:0 14px 44px rgba(16,2,82,0.12)}
.hero .media{position:relative;height:clamp(220px, 40vh, 520px);overflow:hidden} /* responsive hero height */
.hero .media img, .hero .media video{width:100%;height:100%;object-fit:cover;display:block;transform-origin:center center;will-change:transform;transition:transform 0.3s ease;border-radius:12px}
.hero .kenburns{animation:kenburns 20s ease-in-out infinite alternate;}
@keyframes kenburns {
    0% { transform: scale(1) translateY(0px); filter:brightness(.96); }
    100% { transform: scale(1.06) translateY(-4px); filter:brightness(.92); }
}

/* translucent gradient overlay */
.hero .overlay{
    position:absolute;inset:0;background:linear-gradient(180deg, rgba(6,6,24,0.12) 0%, rgba(6,6,24,0.28) 40%, rgba(6,6,24,0.45) 100%);pointer-events:none;
}

/* CTA badges on hero */
.hero .badges{position:absolute;left:14px;top:14px;display:flex;gap:8px;z-index:6}
.badge{background:rgba(255,255,255,0.08);color:#fff;padding:6px 10px;border-radius:10px;backdrop-filter: blur(6px);display:inline-flex;align-items:center;gap:8px;font-weight:700;border:1px solid rgba(255,255,255,0.06);font-size:0.9rem}
.hero .hero-title{position:absolute;left:14px;bottom:14px;color:#fff;z-index:6;max-width:75%}
.hero .hero-title h1{margin:0;font-size:clamp(1.1rem, 2.1vw, 2rem);letter-spacing:0.4px}
.hero .hero-title p{margin:6px 0 0 0;color:rgba(255,255,255,0.95);font-size:0.95rem}
...
.mini-left{display:flex;gap:12px;align-items:center;min-width:0}
.mini-left img{width:56px;height:56px;object-fit:cover;border-radius:8px;flex-shrink:0}
.rating{display:flex;align-items:center;gap:6px;color:var(--primary);font-weight:700}
.actions{display:flex;gap:8px;align-items:center}
.btn-small{padding:8px 10px;border-radius:10px;border:none;font-weight:600;cursor:pointer;font-size:0.95rem}
...
/* Cards */
.card{background:#fff;padding:16px;border-radius:10px;box-shadow:0 8px 30px rgba(16,2,82,0.06)}
.section-title{font-weight:700;color:var(--primary);margin-bottom:10px}

/* Cuisines: small chips with optional images */
.cuisines-grid{display:flex;flex-wrap:wrap;gap:10px;align-items:center}
.cuisine-chip{display:flex;align-items:center;gap:8px;padding:6px 10px;border-radius:999px;background:#fff;border:1px solid #eef4ff;box-shadow:0 6px 18px rgba(22,3,79,0.03);font-weight:600;color:var(--primary)}
.cuisine-chip img{width:28px;height:28px;object-fit:cover;border-radius:6px;flex-shrink:0}

/* gallery */
/* let thumbnails adapt to container width: use auto rows */
.gallery-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:8px}
.gallery-grid img{width:100%;height:clamp(88px, 14vw, 140px);object-fit:cover;border-radius:8px;cursor:pointer;transition:transform .18s ease,box-shadow .18s ease;filter:blur(6px);transform:scale(1.01)}
.gallery-grid img[data-ready="1"]{filter:none;transform:none}
.gallery-grid img:hover{transform:translateY(-6px);box-shadow:0 10px 30px rgba(16,2,82,0.12)}
@media (max-width:980px){ .gallery-grid{grid-template-columns:repeat(2,1fr)} }

/* Swiper - ensure it adapts height gracefully */
.swiper{width:100%;border-radius:8px;overflow:hidden}
.swiper .swiper-slide img{width:100%;height:clamp(200px,28vh,360px);object-fit:cover;display:block}

/* map */
#map{height:300px;border-radius:8px;overflow:hidden}
@media (max-width:640px){ #map{height:220px} }

/* reviews */
.review{border-radius:8px;padding:10px;background:#fbfdff;border:1px solid #eef6ff;display:flex;gap:10px;align-items:flex-start}
.review .meta{font-weight:700;color:var(--primary)}
.review .text{color:#333}

/* collapsible description */
.collapsible{max-height:120px;overflow:hidden;transition:max-height .28s ease}
.collapsible.open{max-height:1200px}

/* TOC sticky */
.toc{position:sticky;top:110px;padding:10px;border-radius:8px;background:#fff;border:1px solid #f1f2f8}
.toc a{display:block;margin-bottom:8px;color:var(--muted);font-weight:600}

/* responsive */
@media(max-width:640px){
    .container{padding:calc(64px + 16px) 12px 40px}
    .navbar a.btn{padding:6px 8px;font-size:0.9rem}
    .mini-card{flex-direction:column;align-items:flex-start;gap:8px;padding:10px}
    .mini-left img{width:48px;height:48px}
    .hero .hero-title{left:12px;bottom:12px;max-width:85%}
    .badge{font-size:0.85rem;padding:6px 8px}
    .swiper .swiper-slide img{height:clamp(160px, 30vw, 260px)}
    .cuisine-chip img{width:24px;height:24px}
}
</style>
</head>
<body>
            ...
            <div class="card" style="margin-top:18px" id="quick-details">
                ...
                <div style="display:flex;gap:12px;flex-wrap:wrap">
                    <div style="min-width:110px">
                        <strong>Type</strong><div class="muted"><?php echo htmlspecialchars($destination['type']); ?></div>
                    </div>
                    <div style="min-width:110px">
                        <strong>Budget</strong><div class="muted">₹<?php echo number_format($destination['budget']); ?> / day</div>
                    </div>
                    <div style="min-width:110px">
                        <strong>Best Season</strong><div class="muted"><?php echo htmlspecialchars($destination['season']); ?></div>
                    </div>
                    <div style="min-width:110px">
                        <strong>Max Travelers</strong><div class="muted">
                        <?php
                        ...
                        if ($peopleArray && is_array($peopleArray)):
                            ...
                            foreach ($peopleArray as $group) {
                                switch ($group) {
                                    case '1': $formattedPeople[] = 'Solo'; break;
                                    case '2': $formattedPeople[] = 'Couples'; break;
                                    case '3-5': $formattedPeople[] = 'Small Groups'; break;
                                    case '6-9': $formattedPeople[] = 'Medium Groups'; break;
                                    case '9+': $formattedPeople[] = 'Large Groups'; break;
                                    default: $formattedPeople[] = $group . ' people';
                                }
                            }
...
thumbs.forEach(img => {
    ...
    img.addEventListener('click', () => {
        ...
        const gallery = pswp.create({ dataSource: items }, options);
        gallery.init();
    });
});
...
favBtn.addEventListener('click', () => {
    ...
    if (!IS_LOGGED_IN) {
        ...
        setFavUI(optimistic);
        favBtn.animate([{ transform: 'scale(1)' }, { transform:'scale(1.04)' }, { transform:'scale(1)'}], { duration:200 });
        return;
    }
    ...
    fetch('actions/toggle_favorite.php', {
      ...
      .catch(()=>{ optimistic = !optimistic; setFavUI(optimistic); alert('Network error'); })
...
shareBtn.addEventListener('click', async () => {
    const shareData = { title: DEST_NAME + ' | TripMate', text: DEST_LOCATION, url: window.location.href };
    if (navigator.share) {
...
if (DEST_LAT !== null && DEST_LNG !== null) {
    try {
        const map = L.map('map', { scrollWheelZoom: false }).setView([DEST_LAT, DEST_LNG], 12);
        ...
        document.getElementById('routeFromBtn')?.addEventListener('click', () => {
            ...
            navigator.geolocation.getCurrentPosition(pos => {
                const fromLat = pos.coords.latitude; const fromLng = pos.coords.longitude;
...
(function loadWeather(){
    ...
    fetch(`https://api.openweathermap.org/data/2.5/onecall?lat=${DEST_LAT}&lon=${DEST_LNG}&units=metric&exclude=minutely,hourly,alerts&appid=${apiKey}`)
        ...
        }).catch(err => { weatherEl.textContent = 'Weather load error'; console.warn(err); });
})();
...
(function loadCurrency(){
    ...
    if (!localCurrency) { el.textContent = '—'; return; }
    ...
    function update() {
        try {
            ...
            if (tz) {
                ...
                el.textContent = new Date(now).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'});
